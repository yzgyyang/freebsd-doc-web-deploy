                          FreeBSD 12.1-RELEASE README

  The FreeBSD Project

   Copyright (c) 2000-2019 The FreeBSD Documentation Project
   ...
   Last modified on 2020-12-18 18:14:18 +0000 by Guangyuan Yang.
   Abstract

   This document gives a brief introduction to FreeBSD 12.1-RELEASE. It
...
1. Introduction
   ...
   This distribution is a release of FreeBSD 12.1-RELEASE, the latest point
  ...
  1.1. About FreeBSD
   ...
   PC hardware (i386), and UltraSPARC(R) machines (sparc64). Versions for the
   ...
   A large collection of third-party ported software (the "Ports Collection")
   is also provided to make it easy to obtain and install all your favorite
   traditional UNIX(R) utilities for FreeBSD. Each "port" consists of a set
   of scripts to retrieve, configure, build, and install a piece of software,
   with a single command. Over 24,000 ports, from editors to programming
   ...
   provided by many commercial versions of UNIX(R). Most ports are also
   available as pre-compiled "packages", which can be quickly installed from
  ...
  1.2. Target Audience
   ...
   11.3-RELEASE) are frequently more appropriate. Releases undergo a period
...
2. Obtaining FreeBSD
  ...
  2.1. CDROM and DVD
   ...
   optional, precompiled "packages" from the FreeBSD Ports Collection, or
  ...
  2.2. FTP
   ...
   section of the Handbook. Finding a close (in networking terms) mirror from
   which to download the distribution is highly recommended.
   ...
   Mirrors generally contain the ISO images generally used to create a CDROM
   of a FreeBSD release. They usually also contain floppy disk images (for
   applicable platforms), as well as the files necessary to do an
   installation over the network. Finally mirrors sites usually contain a set
   of packages for the most current release.
...
3. Contacting the FreeBSD Project
  ...
  3.2. Submitting Problem Reports
   ...
   For more information, "Writing FreeBSD Problem Reports", available on the
...
4. Further Reading
  ...
  4.1. Release Documentation
   ...
   Portable Document Format (.PDF).
     ...
     * RELNOTES.TXT: The release notes, showing what's new and different in
       FreeBSD 12.1-RELEASE compared to the previous release (FreeBSD
       12.0-RELEASE).
   ...
   On platforms that support bsdinstall(8) (currently amd64, i386, and
   sparc64), these documents are generally available via the Documentation
   menu during installation. Once the system is installed, you can revisit
   this menu by re-running the bsdinstall(8) utility.
  ...
  Note:
   ...
   problems. The errata file accompanying each release (most likely right
   next to this file) is already out of date by definition, but other copies
   are kept updated on the Internet and should be consulted as the "current
   errata" for this release. These other copies of the errata are located at
   ../../../../releases/ (as well as any sites which keep up-to-date mirrors
   of this location).
  ...
  4.3. Books and Articles
   ...
   browser to read the Handbook and FAQ locally. In particular, note that the
